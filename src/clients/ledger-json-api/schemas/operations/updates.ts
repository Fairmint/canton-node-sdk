...
export const IdentifierFilterSchema = z.union([
  ...
  z.object({ InterfaceFilter: z.object({
    interfaceId: z.string(),
    includeInterfaceView: z.boolean().optional(),
    includeCreatedEventBlob: z.boolean().optional(),
  }) }),
  z.object({ TemplateFilter: z.object({
    templateId: z.string(),
    includeCreatedEventBlob: z.boolean().optional(),
  }) }),
  z.object({ WildcardFilter: z.object({
    includeCreatedEventBlob: z.boolean().optional(),
  }) }),
]);
...
/**
 * Schema for cumulative filter configuration.
 * Defines filters that accumulate across multiple operations.
 */
...
/**
 * Schema for filters that apply to any party.
 * Defines cumulative filters that can be applied regardless of specific party.
 */
export const FiltersForAnyPartySchema = z.object({
...
/**
 * Schema for event format configuration in operations.
...
/**
 * Schema for transaction shape configuration.
 * Defines the level of detail provided in transaction responses.
 */
export const TransactionShapeSchema = z.union([
  /**
   * AcsDelta: The transaction shape that is sufficient to maintain an accurate ACS view. 
   * This translates to create and archive events. The field witness_parties in events are 
   * populated as stakeholders, transaction filter will apply accordingly.
   */
  z.literal('TRANSACTION_SHAPE_ACS_DELTA'),
  /**
   * LedgerEffects: The transaction shape that allows maintaining an ACS and also conveys 
   * detailed information about all exercises. This translates to create, consuming exercise 
   * and non-consuming exercise. The field witness_parties in events are populated as 
   * cumulative informees, transaction filter will apply accordingly.
   */
...
/**
 * Schema for transaction format configuration.
 * Defines the event format and transaction shape for ledger operations.
 */
...
/**
 * Schema for get updates parameters.
 * Defines parameters for retrieving flat updates from the ledger.
 */
 ...
 * Defines parameters for retrieving update trees from the ledger.
 ...
 * Defines parameters for retrieving a specific transaction by its offset.
 */
 ...
 * Defines parameters for retrieving a specific update by its offset.
 */
...
/**
 * Schema for get transaction by id parameters.
 * Defines parameters for retrieving a specific transaction by its ID.
 */
...
/**
 * Schema for get update by id parameters.
 * Defines parameters for retrieving a specific update by its ID.
 */
...
/**
 * Schema for get transaction tree by id parameters.
 * Defines parameters for retrieving a transaction tree by its ID.
 */
