...
export async function getFeaturedAppRightContractDetails(validatorApi: ValidatorApiClient): Promise<DisclosedContract> {
  ...
  const synchronizerIdFromRules = (amuletRules as any)?.amulet_rules?.domain_id || '';
  return {
    ...
    synchronizerId: (featuredAppRight as any)?.featured_app_right?.domain_id || synchronizerIdFromRules,
