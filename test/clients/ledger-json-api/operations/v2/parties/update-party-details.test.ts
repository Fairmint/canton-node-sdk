...
import { UpdatePartyDetails } from '../../../../../../src/UpdatePartyDetails';
import { mockClientConfig, mockApiUrls } from '../../../../../config/testConfig';

describe('UpdatePartyDetails Operation', () => {
  let mockClient: MockLedgerJsonApiClient;
  let operation: InstanceType<typeof UpdatePartyDetails>;

  beforeEach(() => {
    mockClient = new MockLedgerJsonApiClient(mockClientConfig);
    operation = new UpdatePartyDetails(mockClient);
  ...
  describe('Network Activity Validation', () => {
    it('should make a PATCH request to the correct endpoint', async () => {
      ...
      const params = {};
      const mockResponse = {};
      const expectedUrl = `${mockApiUrls.LEDGER_JSON_API}/v2/parties/{party}`;
