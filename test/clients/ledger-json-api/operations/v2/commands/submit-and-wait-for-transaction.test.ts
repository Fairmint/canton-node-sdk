...
describe('SubmitAndWaitForTransaction Operation', () => {
  let mockClient: MockLedgerJsonApiClient;
  let operation: InstanceType<typeof SubmitAndWaitForTransaction>;

  beforeEach(() => {
    mockClient = new MockLedgerJsonApiClient(mockClientConfig);
    operation = new SubmitAndWaitForTransaction(mockClient);
  ...
  describe('Network Activity Validation', () => {
    it('should make a POST request to the correct commands endpoint', async () => {
      ...
      const expectedUrl = `${mockApiUrls.LEDGER_JSON_API}/v2/commands/submit-and-wait-for-transaction`;
      ...
      expect(request!.data).toMatchObject({
        commands: expect.objectContaining({
          command: params.commands.command,
          meta: params.commands.meta,
        }),
        transactionFormat: params.transactionFormat,
      });
      expect((request!.data as any).commands.commandId).toBeDefined();
    });
  ...
  describe('Parameter Validation', () => {
    ...
    it('should handle different command types correctly', async () => {
      ...
      const request = mockClient.getLastRequest();
      expect(request).toBeDefined();
      expect((request?.data as any)?.commands.command.type).toBe('Exercise');
    });
  });
}); 