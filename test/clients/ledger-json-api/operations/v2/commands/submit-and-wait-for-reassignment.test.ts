...
import { SubmitAndWaitForReassignment } from '../../../../../../src/clients/ledger-json-api/operations/v2/commands/submit-and-wait-for-reassignment';
import { mockClientConfig, mockApiUrls } from '../../../../../config/testConfig';

describe('SubmitAndWaitForReassignment Operation', () => {
  let mockClient: MockLedgerJsonApiClient;
  let operation: InstanceType<typeof SubmitAndWaitForReassignment>;

  beforeEach(() => {
    mockClient = new MockLedgerJsonApiClient(mockClientConfig);
    operation = new SubmitAndWaitForReassignment(mockClient);
  ...
  describe('Network Activity Validation', () => {
    it('should make a POST request to the correct commands endpoint', async () => {
      ...
      const expectedUrl = `${mockApiUrls.LEDGER_JSON_API}/v2/commands/submit-and-wait-for-reassignment`;
    ...
    it('should include proper headers in the request', async () => {
      ...
      const mockResponse = {
        reassignment: {
          reassignmentId: 'reassignment-456',
          status: 'Committed',
        },
      };
      const expectedUrl = `${mockApiUrls.LEDGER_JSON_API}/v2/commands/submit-and-wait-for-reassignment`;
  ...
  describe('Parameter Validation', () => {
    ...
    it('should handle different command types correctly', async () => {
      ...
      const mockResponse = {
        reassignment: {
          reassignmentId: 'reassignment-mixed',
          status: 'Committed',
        },
      };
      const expectedUrl = `${mockApiUrls.LEDGER_JSON_API}/v2/commands/submit-and-wait-for-reassignment`;
