...
import { RevokeUserRights } from '../../../../../../src/RevokeUserRights';
import { mockClientConfig, mockApiUrls } from '../../../../../config/testConfig';

describe('RevokeUserRights Operation', () => {
  let mockClient: MockLedgerJsonApiClient;
  let operation: InstanceType<typeof RevokeUserRights>;

  beforeEach(() => {
    mockClient = new MockLedgerJsonApiClient(mockClientConfig);
    operation = new RevokeUserRights(mockClient);
  ...
  describe('Network Activity Validation', () => {
    it('should make a PATCH request to the correct endpoint', async () => {
      ...
      const params = {};
      const mockResponse = {};
      const expectedUrl = `${mockApiUrls.LEDGER_JSON_API}/v2/users/{user-id}/rights`;
