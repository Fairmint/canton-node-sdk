/**
 * Example 2: Grant Read Rights for External Party
 *
 ...
 *
 * Usage:
 *   npx tsx examples/external-signing/02-grant-external-party-read-rights.ts <key-file>
 *
 * Example:
 *   npx tsx examples/external-signing/02-grant-external-party-read-rights.ts ../keys/alice--12abc.json
 */
...
async function main() {
  ...
  if (!keyFilePath) {
    ...
    console.error('Usage: npx tsx 02-grant-external-party-read-rights.ts <key-file>');
    console.error('Example: npx tsx 02-grant-external-party-read-rights.ts ../keys/alice--12abc.json');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('\n💡 You can now proceed to create and accept transfer offers:');
    console.log(`   npx tsx examples/external-signing/03-create-transfer-offer.ts ${keyFilePath} 10.0`);
    ...
    if (error.status === 403) {
      ...
      console.log('   });');
      console.log('   ```\n');
      console.log('3. Once rights are granted, proceed to steps 3 and 4:');
      console.log(`   npx tsx examples/external-signing/03-create-transfer-offer.ts ${keyFilePath} 10.0`);
      console.log(`   npx tsx examples/external-signing/04-accept-transfer-offer.ts ${keyFilePath}`);
      console.log('');
      process.exit(1);
