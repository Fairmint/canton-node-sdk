 ...
 * 2. Load the transfer offer info (created by example 02)
 * 3. Prepare a transaction to accept the offer
 * 4. Sign the transaction with the external key
 * 5. Execute the signed transaction
 *
 * Usage:
 *   ts-node examples/external-signing/03-accept-transfer-offer.ts <party-id-file>
 *
 * Example:
 *   ts-node examples/external-signing/03-accept-transfer-offer.ts keys/alice--12abc.json
 *
 * Note: Run example 02 first to create a transfer offer!
 */
...
async function main() {
  ...
  if (!keyFilePath) {
    ...
    console.error('Usage: ts-node 03-accept-transfer-offer.ts <key-file>');
    console.error('Example: ts-node 03-accept-transfer-offer.ts keys/alice--12abc.json');
    process.exit(1);
  }
